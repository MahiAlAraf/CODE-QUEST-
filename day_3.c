...
void new_game() {
    ...
    while (1) {
        ...
        // ---- SHOP / TRADE EVENT GOES HERE (before Function Falls) ----
        // At this point coins earned so far (perfect run) total ~117,
        // which is enough for ONE Alexure (+20 HP) at ~100 coins,
        // but not enough for any pricier item (150+). This is intentional.
...
int variable_village() {
    ...
    correct_answer(Q1, Q1_option, 2, 10, 3, 3, Q1_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q2, Q2_option, 1, 8, 3, 3, Q2_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q4, Q4_option, 3, 20, 6, 6, Q4_explanation);
    if (HP <= 0) return HP;
...
int loop_forest() {
    ...
    correct_answer(Q1, Q1_option, 1, 10, 12, 5, Q1_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q2, Q2_option, 1, 10, 12, 6, Q2_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q3, Q3_option, 3, 10, 14, 7, Q3_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q4, Q4_option, 2, 10, 18, 9, Q4_explanation);
    if (HP <= 0) return HP;
...
void After_forest_EQuest() {
    ...
    if (choice == 1) {
        ...
        correct_answer(side_Q1, side_q1_options, 1, 0, 15, 5, side_Q1_explanation);
...
int array_cave() {
    ...
    correct_answer(Q1, Q1_option, 3, 10, 8, 10, Q1_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q2, Q2_option, 2, 12, 10, 10, Q2_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q3, Q3_option, 3, 15, 12, 10, Q3_explanation);
    if (HP <= 0) return HP;
...
int Function_falls() {
    ...
    correct_answer(Q1, Q1_option, 2, 15, 40, 10, Q1_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q2, Q2_option, 3, 18, 50, 12, Q2_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q3, Q3_option, 4, 20, 60, 14, Q3_explanation);
    if (HP <= 0) return HP;
    ...
    correct_answer(Q4, Q4_option, 2, 22, 70, 16, Q4_explanation);
    if (HP <= 0) return HP;
